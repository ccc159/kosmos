...
export default function Exhibit({ piece, extended }: ExhibitProps) {
  return (
    <motion.div className='flex flex-col h-full bg-white z-30' variants={containerVariants} initial={extended ? 'animate' : 'initial'} animate={extended ? 'initial' : 'animate'}>
      <motion.img
      ...
      />
      <motion.div className='flex flex-col h-full font-montserrat' variants={listVariants} initial={extended ? 'animate' : 'initial'} animate={extended ? 'initial' : 'animate'}>
